...
export default new Vuex.Store({
        ...
        PATH: function(state) {
            if (state.setting == "dev") {
                ...
                if (state.environment == "test") {
                    return (
                        state.account.currentUrlHead +
                        "//test.api.zhishutianxia.com"
                    );
                } else {
    ...
    mutations: {
        ...
        drawlight: function(state, e) {
            ...
            if (state.isshow.islightshow == false) {
                ...
                if (document.getElementById(e) != null) {
                    lightChart = echarts.getInstanceByDom(
        ...
        setlightDate: function(state) {
            state.market.jsonTow.Parameters =
                state.market.currentdetail.LastQuotation;
            var TimeLength = state.market.lightChartTime.time.length;
            state.market.lightChartTime.price.push(
                state.market.jsonTow.Parameters.LastPrice.toFixed(
                    state.market.currentdetail.DotSize
                )
            );
            state.market.lightChartTime.time.push(
                state.market.jsonTow.Parameters.DateTimeStamp.split(" ")[1]
            );
            state.market.lightChartTime.time = state.market.lightChartTime.time.slice(
                -50
            );
            state.market.lightChartTime.price = state.market.lightChartTime.price.slice(
                -50
            );
        ...
        setklineoption: function(state) {
            ...
            if (state.market.jsonDataKline.Parameters.HisQuoteType == 1440) {
                for (var i = 0; i < Len; i++) {
                    ...
                    var openPrice = parseFloat(parameters[i][2]).toFixed(
                        dosizeL
                    );
                    var closePrice = parseFloat(parameters[i][1]).toFixed(
                        dosizeL
                    );
                    var sgData = [
                        timeStr,
                        openPrice,
                        closePrice,
                        parseFloat(parameters[i][3]).toFixed(dosizeL),
                        parseFloat(parameters[i][4]).toFixed(dosizeL),
                        parameters[i][2]
                    ];
            ...
            for (var i = 0; i < rawData.slice(-40).length; i++) {
                ...
                values.push([
                    rawData.slice(-40)[i][1],
                    rawData.slice(-40)[i][2],
                    rawData.slice(-40)[i][3],
                    rawData.slice(-40)[i][4]
                ]);
            ...
            state.market.option3 = {
                ...
                tooltip: {
                    ...
                    formatter: function(params) {
                        ...
                        res +=
                            "<br/>  开盘 : " +
                            parseFloat(kd[1]).toFixed(dosizeL) +
                            "  最高 : " +
                            parseFloat(kd[4]).toFixed(dosizeL);
                        res +=
                            "<br/>  收盘 : " +
                            parseFloat(kd[2]).toFixed(dosizeL) +
                            " 最低 : " +
                            parseFloat(kd[3]).toFixed(dosizeL);
                        res +=
                            '<br/> <span style="display:inline-block;margin-right:5px;border-radius:10px;width:9px;height:9px;background-color:#3689B3"></span> MA5 : ' +
                            ma5 +
                            '  <span style="display:inline-block;margin-right:5px;border-radius:10px;width:9px;height:9px;background-color:#B236B3"></span> MA10 : ' +
                            ma10;
                        res +=
                            '<br/><span style="display:inline-block;margin-right:5px;border-radius:10px;width:9px;height:9px;background-color:#B37436"></span> MA20 : ' +
                            ma20 +
                            '  <span style="display:inline-block;margin-right:5px;border-radius:10px;width:9px;height:9px;background-color:#B2B336"></span> MA30 : ' +
                            ma30;
                        return res;
                    },
            ...
            var vol = [],
                ...
                time = [];
            var Ktime;
            state.market.jsonDataKline.Parameters.Data.slice(-40).forEach(
                function(e) {
                    vol.push(e[6]);
                    Ktime =
                        e[0].split(" ")[1].split(":")[0] +
        ...
        drawkline: function(state, x) {
            ...
            if (state.isshow.isklineshow == false) {
                ...
                if (document.getElementById(x.id1) != null) {
                    kline = echarts.getInstanceByDom(
                        document.getElementById(x.id1)
                    );
                }
                if (document.getElementById(x.id2) != null) {
                    volume = echarts.getInstanceByDom(
                        document.getElementById(x.id2)
                    );
                }
            }
        ...
        setfensoption: function(state, arr) {
            ...
            if (state.market.jsonData[state.market.currentNo] != undefined) {
                state.market.jsonData[
                    state.market.currentNo
                ].Parameters.Data.forEach(function(e) {
                    vol.push(e[6]);
                    time.push(
                        e[0].split(" ")[1].split(":")[0] +
                            ":" +
    ...
    actions: {
        ...
        handleTradeMessage: function(context, evt) {
            ...
            switch (data.Method) {
                case "OnRtnHeartBeat":
                    context.state.market.HeartBeat.lastHeartBeatTimestamp =
                        parameters.Ref; // 更新心跳最新时间戳
                    break;
                case "OnRspLogin":
                    if (
                        context.state.market.tradeConfig.username == "" &&
                        localStorage.tradeUser != "" &&
                        localStorage.tradeUser != undefined
                    ) {
                        context.state.market.tradeConfig.username = JSON.parse(
                            localStorage.tradeUser
                        ).username;
                    }
                    //回复
                    if (parameters.Code == 0) {
                        ...
                        //初始资金
                        context.state.market.initBalance =
                            parameters.InitBalance;
                        //查询持仓合计
                        context.state.tradeSocket.send(
                            '{"Method":"QryHoldTotal","Parameters":{"ClientNo":"' +
                                context.state.market.tradeConfig.username +
                                '"}}'
                        );
                        //查询订单
                        context.state.tradeSocket.send(
                            '{"Method":"QryOrder","Parameters":{"ClientNo":"' +
                                context.state.market.tradeConfig.username +
                                '"}}'
                        );
                        //查询成交记录
                        context.state.tradeSocket.send(
                            '{"Method":"QryTrade","Parameters":{"ClientNo":"' +
                                context.state.market.tradeConfig.username +
                                '"}}'
                        );
                        //查询账户信息
                        context.state.tradeSocket.send(
                            '{"Method":"QryAccount","Parameters":{"ClientNo":"' +
                                context.state.market.tradeConfig.username +
                                '"}}'
                        );
                        //查询止损单
                        context.state.tradeSocket.send(
                            '{"Method":"QryStopLoss","Parameters":{"ClientNo":"' +
                                context.state.market.tradeConfig.username +
                                '"}}'
                        );
                        //查询条件单
                        context.state.tradeSocket.send(
                            '{"Method":"QryCondition","Parameters":{"ClientNo":"' +
                                context.state.market.tradeConfig.username +
                                '"}}'
                        );
                        //查询历史成交
                ...
                case "OnRspQryTrade": //查询成交记录回复
                    ...
                    if (parameters != null) {
                        context.state.market.OnRspQryTradeDealListCont.push(
                            parameters
                        );
                    }
                    break;
                case "OnRspQryAccount": //查询账户信息回复
                    ...
                    if (
                        ...
                        context.state.market.CacheAccount.moneyDetail.push(
                            parameters
                        );
                ...
                case "OnRtnOrderTraded":
                    if (parameters != null) {
                        context.state.market.OnRspQryTradeDealListCont.unshift(
                            parameters
                        );
                    }
                ...
                case "OnRspQryStopLoss":
                    if (parameters != null) {
                        if (parameters.Status == 0 || parameters.Status == 1) {
                            ...
                            context.state.market.stopLossTriggeredList.push(
                                parameters
                            );
                        }
                    }
                ...
                case "OnRspQryCondition":
                    if (parameters != null) {
                        if (parameters.Status < 2) {
                            ...
                            context.state.market.triggerConditionList.push(
                                parameters
                            );
                        }
                    }
                ...
                case "OnRtnConditionState":
                    if (parameters.Status == 0) {
                        Toast({
                            message:
                                "【" +
                                parameters.CommodityNo +
                                parameters.ContractNo +
                                "】条件单【" +
                                parameters.ConditionNo +
                                "】,改单后已运行",
                            position: "bottom",
                            duration: 1000
                        });
        ...
        dealWithOnRtnConditionState: function(context, parameters) {
            context.state.market.conditionList.forEach(function(e, i) {
                if (parameters.ConditionNo == e.ConditionNo) {
                    ...
                    if (parameters.Status < 2) {
                        context.state.market.conditionList.splice(
                            i,
                            1,
                            parameters
                        );
        ...
        updateStopLoss: function(context, parameters) {
            setTimeout(() => {
                Toast({
                    message: "单号【" + parameters.StopLossNo + "】,更新成功",
                    position: "bottom",
                    duration: 1000
                });
            }, 1000);
        ...
        layerOnRspInsertStopLoss: function(context, parameters) {
            if (parameters.Status == 0) {
                Toast({
                    message: "提交成功,单号【" + parameters.StopLossNo + "】",
                    position: "bottom",
                    duration: 1000
                });
            } else {
                Toast({
                    message: "提交失败,原因:【" + parameters.StatusMsg + "】",
                    position: "bottom",
                    duration: 1000
                });
            }
        ...
        qryHisTrade: function(context) {
            ...
            context.state.tradeSocket.send(
                '{"Method":"QryHisTrade","Parameters":{"ClientNo":"' +
                    JSON.parse(localStorage.tradeUser).username +
                    '","BeginTime":"' +
                    beginTime +
                    '","EndTime":"' +
                    endTime +
        ...
        layerOnRtnOrderTraded: function(context, parameters) {
            if (parameters != null) {
                var CommodityName =
                    context.state.market.orderTemplist[parameters.CommodityNo]
                        .CommodityName;
                var DirectionStr;
                ...
                var TradePrice = parseFloat(parameters.TradePrice).toFixed(
                    context.state.market.orderTemplist[parameters.CommodityNo]
                        .DotSize
                );
                setTimeout(() => {
        ...
        layerMessage: function(context, parameters) {
            if (parameters != null) {
                ...
                if (parameters.OrderPriceType == 0) {
                    price = parseFloat(parameters.OrderPrice).toFixed(
                        context.state.market.orderTemplist[
                            parameters.CommodityNo
                        ].DotSize
                    );
                }
                ...
                if (parameters.OrderStatus < 4) {
                    ...
                    //存本地
                    let subscribeOrder = localStorage.subscribeOrder
                        ? JSON.parse(localStorage.subscribeOrder)
                        : "";
                    if (subscribeOrder == "") {
                        ...
                        subscribeOrder.forEach((o, i) => {
                            if (o.name != parameters.CommodityNo)
                                subscribeOrder.push(_subscribeOrder);
                        });
                    }
                    localStorage.subscribeOrder = JSON.stringify(
                        subscribeOrder
                    );
                } else {
                    Toast({
        ...
        updateApply: function(context, parameters) {
            ...
            if (parameters.OrderStatus < 3) {
                ...
                if (isExist == true) {
                    ...
                    currentObj.ApplyOrderNum =
                        parameters.OrderNum - parameters.TradeNum;
                    context.state.market.orderListCont.splice(
                        index,
                        1,
                        currentObj
                    );
                ...
                context.state.market.OnRspOrderInsertOrderListCont.splice(
                    context.state.market.OnRspOrderInsertOrderListCont.length -
                        index -
                        1,
                    1
                );
        ...
        appendApply: function(context, parameters) {
            if (parameters.OrderStatus < 3) {
                // 订单已提交、排队中、部分成交 显示到挂单列表
                context.state.market.OnRspOrderInsertOrderListCont.push(
                    parameters
                );
            }
        },
        appendOrder00: function(context, parameters) {
            context.state.market.OnRspOrderInsertEntrustCont.unshift(
                parameters
            );
        },
        ...
        updateOrder: function(context, parameters) {
            context.state.market.OnRspOrderInsertEntrustCont.forEach(function(
                e,
                i
            ) {
                if (e.OrderID == parameters.OrderID) {
                    ...
                    context.state.market.OnRspOrderInsertEntrustCont.splice(
                        i,
                        1,
                        parameters
                    );
        ...
        updateHold: function(context, parameters) {
            ...
            if (isExist == false) {
                ...
                obj.price = parameters.HoldAvgPrice.toFixed(
                    context.state.market.orderTemplist[parameters.CommodityNo]
                        .DotSize
                );
                obj.total = 0;
                ...
                obj.commodityNocontractNo =
                    context.state.market.orderTemplist[parameters.CommodityNo]
                        .LastQuotation.CommodityNo +
                    context.state.market.orderTemplist[parameters.CommodityNo]
                        .LastQuotation.ContractNo;
            ...
            if (isExist == true) {
                if (parameters.HoldNum != 0) {
                    ...
                    positionListContCurrent.price = parseFloat(
                        parameters.OpenAvgPrice
                    ).toFixed(
                        context.state.market.orderTemplist[
                            parameters.CommodityNo
                        ].DotSize
                    );
                    context.state.market.positionListCont.splice(
                        positionListContCurrentIndex,
                        1,
                        positionListContCurrent
                    );
                    context.state.market.qryHoldTotalArr[
                        context.state.market.qryHoldTotalArr.length -
                            1 -
                            positionListContCurrentIndex
                    ] = parameters;
                } else {
                    ...
                    context.state.market.qryHoldTotalArr.splice(
                        context.state.market.qryHoldTotalArr.length -
                            1 -
                            positionListContCurrentIndex,
                        1
                    ...
                    if (context.state.market.positionListCont.length == 0) {
                        let jCacheAccount =
                            context.state.market.CacheAccount.jCacheAccount;
                    ...
                    context.state.market.CacheHoldFloatingProfit.jHoldFloatingProfit[
                        parameters.CommodityNo + parameters.ContractNo
                    ] = {
                        currencyNo:
                            context.state.market.orderTemplist[
                                parameters.CommodityNo
        ...
        updateAccountFloatingProfit: function(context, parameters) {
            if (context.state.market.ifUpdateAccountProfit) {
                //遍历持仓浮盈，根据币种合并逐笔浮盈
                for (var contract in context.state.market
                    .CacheHoldFloatingProfit.jHoldFloatingProfit) {
                    var contractFloatingProfit =
                        context.state.market.CacheHoldFloatingProfit
                            .jHoldFloatingProfit[contract];
                    var obj =
                        context.state.market.CacheHoldFloatingProfit
                            .jCurrencyNoFloatingProfit[
                            contractFloatingProfit.currencyNo
                        ];
                    if (
                        obj == null ||
                        ...
                        if (
                            isNaN(contractFloatingProfit.floatingProfit) ||
                            !contractFloatingProfit.floatingProfit
                        ) {
                ...
                for (var currencyNo in context.state.market
                    .CacheHoldFloatingProfit.jCurrencyNoFloatingProfit) {
                    var floatingProfit = parseFloat(
                        context.state.market.CacheHoldFloatingProfit
                            .jCurrencyNoFloatingProfit[currencyNo]
                    );
                    var todayAmount = parseFloat(
                        context.state.market.CacheAccount.jCacheAccount[
                            currencyNo
                        ].TodayAmount
                    );
                    ...
                    // 可用资金 = 当前权益 - 保证金 - 冻结资金
                    var deposit = parseFloat(
                        context.state.market.CacheAccount.jCacheAccount[
                            currencyNo
                        ].Deposit
                    );
                    var frozenMoney = parseFloat(
                        context.state.market.CacheAccount.jCacheAccount[
                            currencyNo
                        ].FrozenMoney
                    );
        ...
        updateCacheAccount: function(context, parameters) {
            ...
            context.state.market.CacheAccount.jCacheAccount[
                ...
                parameters.CloseProfit;
            // 今结存
            context.state.market.CacheAccount.jCacheAccount[
                parameters.CurrencyNo
            ].TodayAmount =
                parameters.TodayAmount;
            // 保证金
            context.state.market.CacheAccount.jCacheAccount[
                parameters.CurrencyNo
            ].Deposit =
        ...
        updateTotalAccount: function(context, parameters) {
            if (parameters != null) {
                context.state.market.CacheAccount.jCacheAccount[
                    ...
                    parameters.floatingProfit;
                context.state.market.CacheAccount.moneyDetail.forEach(function(
                    o,
                    i
                ) {
            ...
            for (var e in obj) {
                ...
                context.state.market.CacheAccount.jCacheTotalAccount.FloatingProfit +=
                    floatingProfit * obj[e].CurrencyRate;
                // 今权益 = 今结存 + 浮盈
                var tmpTodayBalance = obj[e].TodayAmount + floatingProfit;
                context.state.market.CacheAccount.jCacheTotalAccount.TodayBalance +=
                    tmpTodayBalance * obj[e].CurrencyRate;
                // 今可用=今权益-冻结资金-保证金
                context.state.market.CacheAccount.jCacheTotalAccount.TodayCanUse +=
                    (tmpTodayBalance - obj[e].FrozenMoney - obj[e].Deposit) *
                    obj[e].CurrencyRate;
                // 平仓盈亏
                context.state.market.CacheAccount.jCacheTotalAccount.CloseProfit +=
                    obj[e].CloseProfit * obj[e].CurrencyRate;
                // 冻结资金
                context.state.market.CacheAccount.jCacheTotalAccount.FrozenMoney +=
                    obj[e].FrozenMoney * obj[e].CurrencyRate;
                // 保证金
                context.state.market.CacheAccount.jCacheTotalAccount.Deposit +=
                    obj[e].Deposit * obj[e].CurrencyRate;
                // 手续费
                context.state.market.CacheAccount.jCacheTotalAccount.CounterFee +=
                    obj[e].CounterFee * obj[e].CurrencyRate;
            }
            // 风险率 = 保证金 / 今权益 * 100%
            context.state.market.CacheAccount.jCacheTotalAccount.RiskRate =
                context.state.market.CacheAccount.jCacheTotalAccount.Deposit /
                context.state.market.CacheAccount.jCacheTotalAccount
                    .TodayBalance /
                100;
            //风险度 =（初始资金 - 今权益）/（初始资金 - 强平线）
            context.state.market.CacheAccount.jCacheTotalAccount.RiskDegree =
                (context.state.market.initBalance -
                    context.state.market.CacheAccount.jCacheTotalAccount
                        .TodayBalance) /
                (context.state.market.initBalance -
                    context.state.market.forceLine) *
                100;
            if (
                context.state.market.CacheAccount.jCacheTotalAccount
                    .RiskDegree < 0
        ...
        UpdateHoldProfit: function(context, parameters) {
            if (context.state.market.ifUpdateHoldProfit) {
                if (parameters != null) {
                    ...
                    context.state.market.positionListCont.forEach(function(
                        e,
                        i
                    ) {
                        if (
                            e.commodityNocontractNo ==
                            parameters.CommodityNo + parameters.ContractNo
                        ) {
                    ...
                    if (currentPositionListContObj != undefined) {
                        ...
                        var diff =
                            ...
                            currentPositionListContObj.price;
                        // 合约乘数 = 最小变动价格 / 最小变动点数
                        var mult =
                            context.state.market.orderTemplist[
                                parameters.CommodityNo
                        ...
                        var tmpFloatingProfit = parseFloat(
                            diff * mult * currentPositionListContObj.HoldNum
                        ).toFixed(2);
                        if (currentPositionListContObj.type == "空") {
                            // 空反向
                        ...
                        context.state.market.positionListCont.splice(
                            currentPositionListContObjIndex,
                            1,
                            currentPositionListContObj
                        );
                        context.state.market.CacheHoldFloatingProfit.jHoldFloatingProfit[
                            parameters.CommodityNo + parameters.ContractNo
                        ] = {
                            ...
                            floatingProfit: tmpFloatingProfit
                        };
                        //更新账户资金盈亏
                        context.dispatch(
        ...
        updateHoldFloatingProfit: function(context, parameters) {
            ...
            // 浮动盈亏 = (价差/最小变动) * (合约乘数 * 最小变动) * 手数 = 价差 * 合约乘数(最小变动价格 / 最小变动点数) * 手数
            var tmpFloatingProfit = parseFloat(
                diff * mult * currentObj.HoldNum
            ).toFixed(2);
            if (currentObj.Drection === 1) {
                // 空反向
                tmpFloatingProfit = -tmpFloatingProfit;
            }
            var floatingProfit =
        ...
        HeartBeatTimingCheck: function(context) {
            context.state.market.HeartBeatTiming = setInterval(
                heartBeatUpdate,
                15000
            );
            function heartBeatUpdate() {
                if (
                    context.state.market.HeartBeat.lastHeartBeatTimestamp ==
                    context.state.market.HeartBeat.oldHeartBeatTimestamp
                ) {
                    if (
                        localStorage.tradeUser == undefined ||
                        localStorage.tradeUser == ""
                    ...
                    context.state.market.HeartBeat.oldHeartBeatTimestamp =
                        context.state.market.HeartBeat.lastHeartBeatTimestamp; // 更新上次心跳时间
                }
            }
            //			heartBeatUpdate();
        },
        ...
        initTrade: function(context) {
            if (
                (localStorage.tradeUser != "" &&
                    localStorage.tradeUser != undefined) ||
                (context.state.market.tradeConfig.username != "" &&
                    context.state.market.tradeConfig.username != undefined)
            ) {
                context.state.tradeSocket = new WebSocket(
                    context.state.market.tradeConfig.url_real
                );
                context.state.tradeSocket.onopen = function(evt) {
                    //登录
                    //					console.log(JSON.parse(localStorage.tradeUser).password);
                    if (context.state.tradeSocket.readyState == 1) {
                        //连接已建立，可以进行通信。
                        if (
                            localStorage.tradeUser != "" &&
                            localStorage.tradeUser != undefined
                            ...
                            context.state.tradeSocket.send(
                                '{"Method":"Login","Parameters":{"ClientNo":"' +
                                    JSON.parse(localStorage.tradeUser)
                                        .username +
                                    '","PassWord":"' +
                                    JSON.parse(localStorage.tradeUser)
                                        .password +
                                    '","IsMock":' +
                                    context.state.market.tradeConfig.model +
                                    ',"Version":"' +
                                    context.state.market.tradeConfig.version +
                                    '","Source":"' +
                                    context.state.market.tradeConfig
                                        .client_source +
                                    '"}}'
                            );
                        } else {
        ...
        initQuoteClient: function(context) {
            context.state.quoteSocket = new WebSocket(
                context.state.market.quoteConfig.url_real
            );
            context.state.quoteSocket.onopen = function(evt) {
                //				console.log('open');
                context.state.quoteSocket.send(
                    '{"Method":"Login","Parameters":{"UserName":"' +
                        context.state.market.quoteConfig.userName +
                        '","PassWord":"' +
                        context.state.market.quoteConfig.passWord +
                        '"}}'
                );
            };
            ...
            context.state.quoteSocket.onmessage = function(evt) {
                ...
                if (context.state.wsjsondata.Method == "OnRspLogin") {
                    // 登录行情服务器
                    Toast({
                        message: "行情服务器连接成功",
                        position: "bottom",
                    ...
                    // 查询服务器支持品种用于订阅
                    context.state.quoteSocket.send(
                        '{"Method":"QryCommodity","Parameters":{' + null + "}}"
                    );
                } else if (
                    context.state.wsjsondata.Method == "OnRspQryCommodity"
                ) {
                    // 行情服务器支持的品种
                    // 行情服务器支持的品种
                    context.state.market.markettemp = JSON.parse(
                        evt.data
                    ).Parameters;
                    ...
                    if (context.state.market.commodityOrder) {
                        context.state.market.commodityOrder.forEach((o, i) => {
                            context.state.quoteSocket.send(
                                '{"Method":"Subscribe","Parameters":{"ExchangeNo":"' +
                                    o.exchangeNo +
                                    '","CommodityNo":"' +
                                    o.commodityNo +
                                    '","ContractNo":"' +
                    ...
                    context.state.market.Parameters.forEach(function(o, i) {
                        context.state.market.commodityOrder.forEach(function(
                            v
                        ) {
                ...
                } else if (context.state.wsjsondata.Method == "OnRtnQuote") {
                    // 最新行情
                    ...
                    context.state.market.Parameters.forEach(function(a, r) {
                        if (a.CommodityNo == key) {
                            if (
                                JSON.parse(evt.data).Parameters.LastPrice >
                                a.LastQuotation.LastPrice
                            ) {
                    ...
                    //合约成交明细
                    var _dealDetails00 = {};
                    context.state.market.tradeParameters.forEach(function(
                        o,
                        i
                    ) {
                        if (key == o.CommodityNo) {
                            _dealDetails00["time"] = val.DateTimeStamp.split(
                                " "
                            )[1];
                            _dealDetails00["price"] = val.LastPrice;
                            _dealDetails00["volume"] = val.LastVolume;
                            _dealDetails00["_price"] = val.PreSettlePrice;
                            _dealDetails00["dotSize"] =
                                context.state.market.orderTemplist[key].DotSize;
                            if (o.data.length >= 12) {
                                o.data.shift();
                            }
                    ...
                    context.state.market.markettemp.forEach(function(e, i) {
                        ...
                        if (
                            JSON.parse(evt.data).Parameters.CommodityNo ==
                            e.CommodityNo
                        ) {
                            //就把拿到数据存入缓存中
                            ...
                            context.state.market.orderTemplist[key] = e;
                            //将显示数据进行更新
                            context.state.market.Parameters.forEach(function(
                                a,
                                r
                            ) {
                                if (a.CommodityNo == e.CommodityNo) {
                                    context.state.market.Parameters.splice(
                                        r,
                                        1,
                                        e
                                    );
                            ...
                            if (
                                context.state.market.currentNo == e.CommodityNo
                            ) {
                                context.state.market.CacheLastQuote.push(
                                    JSON.parse(evt.data).Parameters
                                );
                                if (
                                    context.state.market.CacheLastQuote.length >
                                    2
                                ) {
                                ...
                                //更新分时图
                                if (
                                    context.state.isshow.isfensshow == true &&
                                    context.state.isshow.isfens == true
                                ) {
                                    var arr = [],
                                        arr1,
                                        arr2,
                                    ...
                                    context.state.market.charttime =
                                        context.state.market.charttimems -
                                        context.state.market.charttimems2;
                                    if (
                                        context.state.market.charttime >=
                                            1000 ||
                                        context.state.market.charttimetemp >=
                                            1000
                                    ) {
                                        arr = [];
                                        arr[0] = JSON.parse(
                                        ...
                                        arr1 = JSON.parse(
                                            evt.data
                                        ).Parameters.DateTimeStamp.split(" ");
                                        arr2 = arr1[1].split(":"); //最新时间
                                        arr3 = context.state.market.jsonData[
                                            context.state.market.currentNo
                                        ].Parameters.Data[
                                            context.state.market.jsonData[
                                                context.state.market.currentNo
                                            ].Parameters.Data.length - 1
                                        ][0].split(" ");
                                        ...
                                        if (arr2[1] == arr4[1]) {
                                            ...
                                            context.state.market.jsonData[
                                                context.state.market.currentNo
                                            ].Parameters.Data.push(arr);
                                        }
                                        context.commit(
                                            "setfensoption",
                                            context.state.market.contrastData
                                        );
                                ...
                                //更新闪电图
                                if (
                                    context.state.isshow.islightshow == true &&
                                    context.state.isshow.islight == true
                                ) {
                                    context.state.market.jsonTow = JSON.parse(
                                        evt.data
                                    );
                                ...
                                //更新K线图
                                if (
                                    context.state.isshow.isklineshow == true &&
                                    context.state.isshow.iskline == true
                                ) {
                                    if (
                                        context.state.market.CacheLastQuote[1]
                                            .TotalVolume <=
                                        context.state.market.CacheLastQuote[0]
                                            .TotalVolume
                                    ) {
                                        return;
                                    }
                                    ...
                                    context.state.market.volume += JSON.parse(
                                        evt.data
                                    ).Parameters.LastVolume;
                                    ...
                                    var arr1 = JSON.parse(
                                        evt.data
                                    ).Parameters.DateTimeStamp.split(" "); //得到的时间
                                    //["20", "47", "38"]
                                    var arr2 = arr1[1].split(":"); //得到的数据
                                    var arr3 = context.state.market.jsonDataKline.Parameters.Data[
                                        context.state.market.jsonDataKline
                                            .Parameters.Data.length - 1
                                    ][0].split(" ");
                                    //["20", "48", "00"]
                                    ...
                                    if (context.state.market.selectTime == 1) {
                                        if (arr2[1] == arr4[1]) {
                                            arr[0] =
                                                context.state.market.jsonDataKline.Parameters.Data[
                                                    context.state.market
                                                        .jsonDataKline
                                                        .Parameters.Data
                                                        .length - 1
                                            ...
                                            arr[6] =
                                                context.state.market.volume;
                                            var length =
                                                context.state.market
                                                    .jsonDataKline.Parameters
                                                    .Data.length;
                                            context.state.market.jsonDataKline.Parameters.Data.splice(
                                                length - 1,
                                                1,
                                                arr
                                            );
                                            ...
                                            arrTemp[6] = arr[6];
                                            arr = arrTemp;
                                            context.state.market.jsonDataKline.Parameters.Data.push(
                                                arrTemp
                                            );
                                        }
                                    } else if (
                                        context.state.market.selectTime == 5
                                    ) {
                                        if (arr2[1] % 5 != 0) {
                                            arr[0] =
                                                context.state.market.jsonDataKline.Parameters.Data[
                                        ...
                                        context.state.market.selectTime == 15
                                    ) {
                                        if (arr2[1] % 15 != 0) {
                                            arr[0] =
                                                context.state.market.jsonDataKline.Parameters.Data[
                                        ...
                                        context.state.market.selectTime == 30
                                    ) {
                                        if (arr2[1] % 30 != 0) {
                                            arr[0] =
                                                context.state.market.jsonDataKline.Parameters.Data[
                                        ...
                                        context.state.market.selectTime == 1440
                                    ) {
                                        if (
                                            arr2[1] == "00" &&
                                            arr2[2] == "00"
                    ...
                    context.state.wsjsondata.Method == "OnRspQryHistory"
                ) {
                    // 历史行情
                    let data = JSON.parse(evt.data);
                    if (data.Parameters.HisQuoteType == 0) {
                        context.state.market.jsonData[
                            data.Parameters.CommodityNo
                        ] = data;
